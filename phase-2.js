...
async function workout(totalTime) {
  try{
    timeLeftAfterStretching = await stretch(totalTime);
    timeLeftAfterRunning = await runOnTreadmill(timeLeftAfterStretching);
    timeLeftAfterLifting = await liftWeights(timeLeftAfterRunning);
